...
- **Leaderboard.** The 🏆 button ranks every pilot by USDC (fun stable nicknames from the wallet address, your row highlighted, the record cash-out on top). `/api/leaderboard` finds players from the rounds stored in the game and reads every balance in one multicall, cached a few seconds on the CDN.
