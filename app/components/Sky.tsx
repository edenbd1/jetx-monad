    ...
    const frame = (now: number) => {
      ...
      if (crashed) {
        ...
        if (sinceCrash < EXPLOSION_MS) {
          ...
          ctx.fillStyle = `rgba(255, 220, 160, ${0.55 * (1 - k)})`;
          ctx.beginPath();
          ctx.arc(tipX, tipY, 10 + 70 * k, 0, Math.PI * 2);
          ctx.fill();
        }
        // Wall-clock physics so throttled frames don't stretch the explosion.
        const tS = sinceCrash / 1000;
        const life = 1 - sinceCrash / 900;
        if (life > 0) {
          for (const p of particles) {
            ctx.fillStyle = `hsla(${p.hue}, 100%, 60%, ${life})`;
            ctx.beginPath();
            ctx.arc(p.x + p.vx * tS, p.y + p.vy * tS + 60 * tS * tS, p.size, 0, Math.PI * 2);
            ctx.fill();
          }
        }
      } else {
